...
function crearCita($data) {
    ...
    try {
        ...
        if (!$result) throw new Exception("Fallo al insertar la cita.");
...
// [MEJORA SOFT DELETE] Esta es la función clave que modificamos
function obtenerDoctoresConDepartamento() {
    global $pdo;
    // Solo traemos doctores cuyo usuario esté ACTIVO (is_active = 1)
    $stmt = $pdo->query("
...
function cancelarCita($id, $userId) {
    ...
    log_audit($userId, 'error_cancelar_cita', "Intento fallido cancelación ID: $id");
    return false;
}
...
function completarConsulta($data) {
    ...
    try {
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        log_audit($doctorId, 'error_consulta', "Fallo consulta ID $appointmentId: " . $e->getMessage());
...
function obtenerHistorialMedicoPorUsuario($userId) {
    global $pdo;
    $stmt = $pdo->prepare("SELECT notes, created_at FROM medical_records WHERE user_id = ? ORDER BY created_at DESC");
    $stmt->execute([$userId]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}